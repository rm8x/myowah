      ...
      qLabel_("", "Q:")
{
    ...
    //centreFrequencySlider_.setRange(0.1, 2000.0, 0.1);
    centreFrequencySlider_.setRange(400, 4000, 1);
    ...
    // more natural feel
    centreFrequencySlider_.setSkewFactorFromMidPoint(sqrt(1 * 4000.0));
    ...
    // set up OSC listening

    // specify here on which UDP port number to receive incoming OSC messages
    if (! connect (9001))
        showConnectionErrorMessage ("Error: could not connect to UDP port 9001.");
    
    addListener (this, "/myo1/orientation/scaled");
    startTimer(50);
}
...
void WahwahAudioProcessorEditor::resized()
{
    ...
    resizer_->setBounds(getWidth() - 72, getHeight() - 16, 16, 16);
...
void WahwahAudioProcessorEditor::oscMessageReceived (const OSCMessage& message)
{
  //if (message.size() == 1 && message[0].isFloat32())
   centreFrequencySlider_.setValue (jmap(message[0].getFloat32(), 400.0f, 4000.0f));
  //centreFrequencySlider_.setValue (message[0].getFloat32());
     //centreFrequencySlider_.setValue (jlimit (0.0f, 2010.0f, message[0].getFloat32()));
}

void WahwahAudioProcessorEditor::showConnectionErrorMessage (const String& messageText)
{
